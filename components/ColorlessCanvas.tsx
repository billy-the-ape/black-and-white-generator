...
import { loadImagesIntoCanvas } from "./util/loadImage";
...
const ColorlessCanvas: React.FC<ColorlessCanvasProps> = ({
  file,
  image,
  onRemove,
}) => {
  ...
  const [initialBgColor, setInitialBgColor] = useState<RgbaColor | null>({
    r: 255,
    b: 255,
    g: 255,
    a: 1,
  });
  const [initialFgColor, setInitialFgColor] = useState<RgbaColor | null>({
    r: 0,
    b: 0,
    g: 0,
    a: 1,
  });
  const [bgColor, setBgColor] = useState<RgbaColor | null>({
    r: 255,
    b: 255,
    g: 255,
    a: 1,
  });
  const [fgColor, setFgColor] = useState<RgbaColor | null>({
    r: 0,
    b: 0,
    g: 0,
    a: 1,
  ...
  const downloadImage = () => {
    ...
    link.download = `Color me${file ? " - " + file.name : ""}`;
